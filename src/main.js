...
import powerAppsIcon from '../Icons/powerApps.png';
import sublimeIcon from '../Icons/sublime.png';
import vercelIcon from '../Icons/vercel.png';
import visualStudioIcon from '../Icons/visualStudio.png';
  ...
  <div class="site-shell">
    <aside class="sidebar">
      ...
      <nav aria-label="Primary navigation">
        ${navItems.map(([icon, label, href]) => `<a class="nav-link ${label === 'Home' ? 'active' : ''}" href="${href}" data-section="${href.slice(1)}"><i class="ph ph-${icon}"></i><span>${label}</span></a>`).join('')}
      </nav>
...
links.forEach((link) => link.addEventListener('click', () => document.querySelector('.sidebar').classList.remove('open')));
